                ...
                if (valSet != null && !valSet.isEmpty()) {
                    ...
                    if (it.hasNext()) {
                        ...
                        if (keyAtPosition.equals(passwordAttributeName)) {
                            value = it.next();
                            byte[] bSalt = clsLDAPEnc.generateSalt();
                            byte[] bValue = clsLDAPEnc.encryptionPassword1(ALG_SHA1, value, bSalt);
                            value = clsLDAPEnc.encryptionPassword2(ALG_SHA1, bValue, bSalt);
                        } else if (keyAtPosition.equals("saved_consent")) {
                            // saved_consent をチェック／加工する
                ...
                if (it.hasNext()) {
                	   ...
                	   if(key.equals(passwordAttributeName)) {
                		   ...
                		   byte[] bSalt = null;
	                		...
	                		try {
	                			byte[] bVal = clsLDAPEnc.encryptionPassword1(ALG_SHA1, clearPass, bSalt);
	                			val = clsLDAPEnc.encryptionPassword2(ALG_SHA1, bVal, bSalt);
							} catch (NoSuchAlgorithmException e1) {
								// TODO 自動生成された catch ブロック
								e1.printStackTrace();
							}
